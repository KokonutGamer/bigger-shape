...
function CreateAccountPage() {
  ...
  async function handleCreateAccount() {
    ...
    if (
      !Object.values(isValidInput).every(Boolean) ||
      !Object.values(isValidLength).every(Boolean)
    ) {
      return;
    }
  ...
  return (
    <>
      ...
      <div className="flex flex-col items-center justify-center h-[90vh] w-[100vw]">
        <h1 className="text-white text-3xl font-bold mt-[7vh] mb-[5vh]">
          Create Account
        </h1>
        <div
          className="flex flex-col items-center justify-center w-[40vw] h-[60vh] bg-gradient-to-br from-blue-200 to-blue-500
                rounded-lg
                text-black
                shadow-md"
        >
          <InputField
            ...
            sizeRange={{ min: 3, max: 20 }}
            setIsValidLength={setIsValidLength}
            setCurrentValue={setUserName}
          />
          <ErrorMessage
            message="Only Numbers and Letters"
            hidden={isValidInput.userName}
          />
          <ErrorMessage
            message="Must be between 3 and 20 characters"
            hidden={isValidLength.userName}
          />
          <InputField
            type="password"
            id="password"
          ...
          <ErrorMessage
            ...
            hidden={isValidInput.password}
          />
          <ErrorMessage
            message="Must be between 8 and 20 characters"
            hidden={isValidLength.password}
          />
          <InputField
            type="password"
            id="passwordConfirm"
          ...
          <ErrorMessage
            ...
            hidden={isValidInput.confirmPassword}
          />
          ...
          <ErrorMessage
            ...
            hidden={isValidInput.email}
          />
          <ErrorMessage message="Placeholder" hidden={true} />
          {/* placeholder for consistency */}
          <p className="text-red-600 text-sm text-center mt-[2vh]">
            * required field
          </p>
        </div>
